...
if (isset($_POST['xCoord'])){
    $currAtt["X"] = $_POST['xCoord'];
}
if (isset($_POST['CoordinateY'])){
    $currAtt["Y"] = $_POST['CoordinateY'];
}
if (isset($_POST['Radius'])){ 
    $currAtt["R"] = $_POST['Radius'];
}
$currAtt["date"] = date('H:i:s');
if (!isset($_SESSION["attempts"])){
    $_SESSION["attempts"] = array();
}
if ($currAtt["X"] >= 0 and $currAtt["Y"] >= 0){
    if ($currAtt["X"]**2 + $currAtt["Y"]**2 <= $currAtt["R"]**2){
        $currAtt["Is_Hit"] = "Yes";
    } else {
        $currAtt["Is_Hit"] = "No";
    }
} elseif ($currAtt["X"] < 0 and $currAtt["Y"] >= 0){
    if (0.5*$currAtt["X"] + 0.5*$currAtt["R"] >= $currAtt["Y"]){
        $currAtt["Is_Hit"] = "Yes";
    } else{
        $currAtt["Is_Hit"] = "No";
    }
} elseif ($currAtt["X"] <= 0 and $currAtt["Y"] < 0){
    if ($currAtt["X"] >= -$currAtt["R"] and $currAtt["Y"] >= - 0.5 * $currAtt["R"]){
        $currAtt["Is_Hit"] = "Yes";
    } else{
        $currAtt["Is_Hit"] = "No";
    }
...
array_unshift($_SESSION["attempts"], $currAtt);
?>
    ...
    <body>
        <section>
            <table id="generalTable">
                <caption id="header">
                    <?php
                        if ($currAtt["Is_Hit"]=="Yes"){
                            echo "That's <br> a hit!";
                        } else {
                            echo "You've <br> missed!";
                        }
                    ?>
                </caption>
                <tbody>
                            ...
                            <div id="tableWrap" class="scallop">
                                <table id="table">
                                    <tbody>
                                        ...
                                        <?php
                                            foreach($_SESSION["attempts"] as $att){
                                                $X = $att["X"];
                                                $Y = $att["Y"];
                                                $R = $att["R"];
                                                $date = $att["date"];
            ...
            <table id="footer">
                <tbody>
                    ...
                    <tr>
                        <td colspan="4">
                            <?php echo "This page rendered in ".round((microtime(true) - $time_start)*1000, 2)." ms at ".date('m.d.y H:i:s')?>.
                        </td>
                    </tr>
                </tbody>
            </table>
