...
public class Client {
    ...
    public LightAbstractCommand getLightAbstractCommand(ClientStorage clientStorage, SocketChannel socketChannel) throws IOException, ClassNotFoundException {
        try {
                ...
                //!!!!!new block!!!!!!!
                command.setValidArguments(new String[0]);
                command.setValidMovies(new Movie[0]);
    ...
    public void goAction(ClientStorage clientStorage, String host, int port) {
        try {
            ...
            ClientConnecter clientConnecter = new ClientConnecter(this);
            clientConnecter.connect(host, port);
            clientConnecter.clientAuthorization(clientReceiver, clientSender);
            SocketChannel socketChannel = clientConnecter.getSocketChannel();
