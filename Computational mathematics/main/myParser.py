...
def parse_equation(equation, main_martix, d_matrix, size):
    ...
    for var in current_string:
        ...
        var = float(var)
    d_matrix.append(current_string[-1])
    main_martix.append(current_string[0: -1])
...
def parse_console(entity):
    ...
    accuracy = 0
    iterator = 1
    print(f"{bcolors.WARNING}Вводите коэффициенты, включая свободный коэффициент справа от знака '='{bcolors.ENDC}")
    ...
    while True:
        ...
        break
    entity.set_main_matrix(np.array(main_matrix))
    entity.set_d_matrix(np.array(d_matrix))
